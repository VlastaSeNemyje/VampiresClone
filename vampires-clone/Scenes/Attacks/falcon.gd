...
var spr_falcon_reg = preload("res://Textures/Sprites/Weapons/Falconer.png")
var spr_falcon_atk = preload("res://Textures/Sprites/Weapons/FalconerAttack.png")

@onready var player = get_tree().get_first_node_in_group("player")
@onready var sprite = $Sprite2D
@onready var collision = $CollisionShape2D
@onready var AttackTimer = get_node("AttackTimer")
@onready var ChangeDirection = get_node("%ChangeDirection")
@onready var ResetPosTimer = get_node ("%ResetPosTimer")
@onready var snd_falcon = $snd_falcon
...
func enable_attack(atk = true):
	if atk:
		...
		sprite.texture = spr_falcon_atk
	else:
		collision.call_deferred("set", "disabled", true)
		sprite.texture = spr_falcon_reg
